...
const { createLogger, format, transports } = winston;//logger
...
const yaMaps = '***********';//ключ Яндекс.Карты
...
const welcomeMessage = `Приветствую, здесь можно получить погоду по городу ${cityName}, если вы в ином городе, Нажмите на кнопку "Выбрать город"`;
const mainMessage = `Выбран город ${cityName}`;
//Логгирование
...
bot.on('callback_query', query => {
    switch (query.data) {
        case 'mainMenu':
            ...
            break;
        case 'weather':
            bot.sendMessage(query.message.chat.id, 'На какие дни нужен прогноз погоды? ☁🌡',{
                reply_markup:{
...
// Объект для сохранения координат
        ...
        case 'forecast0':
            fetch(url, options)
                ...
                .then(data => {
                    ...
                    const message1 = yaWeTempOfWater ? `В городе ${cityName} на улице${seasonEmoji} ${yaWeTemp}°C, ощущается как ${yaWeTempFeels}°C\n
Осадки:${emoji} Влажность:${humidity}% Температура воды:${yaWeTempOfWater}°C` : `В городе ${cityName} на улице${seasonEmoji} ${yaWeTemp}°C, ощущается как ${yaWeTempFeels}°C\n${emoji} Влажность:${humidity}%`;
        ...
        case 'forecast1':
            fetch(url, options)
                ...
                .then(data => {
                    ...
                    const humidity1 = object.forecast.parts[0].humidity; //Влажность
                    const humidity2 = object.forecast.parts[1].humidity; //Влажность
                    const season = object.fact.season;
                    let seasonEmoji;
                    if (season === 'winter') {
                        seasonEmoji = '⛄❄';
                    ...
                    const yaWeTempFeels1 = object.forecast.parts[0].feels_like; //ощущается как
                    const yaWeTempFeels2 = object.forecast.parts[1].feels_like; //ощущается как
                    const yaWeTempOfWater1 = object.forecast.parts[0].temp_water; // температура воды
                    const yaWeTempOfWater2 = object.forecast.parts[1].temp_water; // температура воды
                    const message2 = yaWeTempOfWater1 && yaWeTempOfWater2 ? `Давление сегодня:${pressMM1}ммрт завтра:${pressMM2}ммрт\nДавление сегодня: ${pressPA1}Па завтра: ${pressPA2}Па\n
